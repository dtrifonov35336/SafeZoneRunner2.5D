...
public class PickupSpawner3D : MonoBehaviour
{
    ...
    [Range(0f, 1f)] public float coinSpawnChance = 0.9f;
    ...
    void SpawnCoin()
    {
        ...
        if (laneIdx == -1) return;
        SpawnPickup(coinPrefab, lanePositions[laneIdx]);
    }
    ...
    void SpawnHeart()
    {
        ...
        int laneIdx = GetFullyFreeLane();
        if (laneIdx == -1) return;
        SpawnPickup(heartPrefab, lanePositions[laneIdx]);
    }
    ...
    bool IsLaneClear(float laneX)
    {
        ...
        Collider[] obstacles = Physics.OverlapSphere(checkPos, obstacleCheckRadius);
        foreach (var col in obstacles)
        {
            if (col == null) continue;
            if (col.GetComponent<ObstacleMover3D>() != null) return false;
        }
    ...
    void SpawnPickup(GameObject prefab, float laneX)
    {
